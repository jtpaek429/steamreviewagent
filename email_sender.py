...
def send_multi_digest(results: list[dict], to_email: str | None = None) -> None:
    ...
    Each entry in results must have:
        app_id      str
        game_name   str
        analysis    dict | None   (None when the game failed)
        trend_spikes list[dict]   (empty list when unavailable)
        error       str | None    (set when analysis is None)
    ...
    n = len(results)
    subject = f"Steam Review Digest — {n} game{'s' if n != 1 else ''} — {_today()}"
    html = _build_multi_html(results)
...
def _build_multi_html(results: list[dict]) -> str:
    """Build a consolidated HTML email with one section per game."""
    succeeded = [r for r in results if r["analysis"] is not None]
    failed = [r for r in results if r["analysis"] is None]

    game_sections = "\n".join(
        _build_game_section(r["game_name"] or f"App ID {r['app_id']}",
                            r["analysis"], r["trend_spikes"])
        for r in succeeded
    )
    ...
    if failed:
        error_items = "".join(
            f'<li><strong>App ID {r["app_id"]}</strong>'
            f'{" (" + r["game_name"] + ")" if r["game_name"] else ""}'
            f' — {r["error"]}</li>'
            for r in failed
        )
        error_section = f"""
<div class="card" style="border-left: 4px solid #ef4444;">
  <h2 style="color:#991b1b;">❌ Failed Games</h2>
  <ul style="font-size:14px;color:#555;margin:0;padding-left:18px;">{error_items}</ul>
</div>"""

    n_ok = len(succeeded)
    n_fail = len(failed)
    summary_line = f"{n_ok} succeeded" + (f", {n_fail} failed" if n_fail else "")
...
<div class="card">
  <h1>🎮 Steam Review Digest</h1>
  <div class="meta">Last 7 days · {_today()} · {summary_line}</div>
</div>
...
def _build_game_section(title: str, analysis: dict, trend_spikes: list[dict]) -> str:
    """Render one game's cards (summary + themes + optional spikes/trends)."""
    ...
    spikes_card = (
        f'<div class="card"><h2>⚠️ Flagged Spikes</h2>{spikes_html}</div>'
        if spikes else ""
    )
    trends_card = (
        f'<div class="card"><h2>📈 Week-over-Week Trends</h2>{trends_html}</div>'
        if trend_spikes else ""
    )
